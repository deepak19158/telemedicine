...
export default function AdminUsersPage() {
  ...
  const { page, limit, nextPage, prevPage, goToPage, params } = usePagination(
    1,
    12
  );
  ...
  const { mutate: updateUserStatus, loading: updatingStatus } =
    useUpdateUserStatus();
    ...
    return (
      <div className="min-h-screen bg-medical-gradient-soft flex items-center justify-center">
        <div className="card-medical text-center max-w-md">
          ...
          <button className="btn-primary" onClick={() => refetchUsers()}>
            Try Again
          </button>
        </div>
      </div>
  ...
  const getRoleBadge = (role: string) => {
    ...
    return (
      <span
        className={`px-2 py-1 rounded-full text-xs font-medium ${colors[role] || colors.patient}`}
      >
                ...
                users.map((user: any) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <span
                        className={`badge-${user.isActive ? "success" : "error"}`}
                      >
                    ...
                    <div className="flex space-x-2">
                      ...
                      <button
                        ...
                        onClick={() =>
                          handleStatusUpdate(user._id, !user.isActive)
                        }

...
export async function POST(request: NextRequest) {
  try {
    ...
    const updatedPatient = await User.findByIdAndUpdate(
      ...
      { new: true }
    ).populate('profile.assignedDoctor', 'profile.name profile.specialization profile.consultationFee')
...
export async function GET(request: NextRequest) {
  try {
    ...
    .select('profile.name profile.specialization profile.consultationFee profile.experience')

    // Calculate assignment statistics
    const assignmentStats = {
      totalPatients: patients.length,
      assignedPatients: patients.filter(p => p.profile?.assignedDoctor).length,
      unassignedPatients: patients.filter(p => !p.profile?.assignedDoctor).length,
    ...
    const patientsData = patients.map(patient => ({
      ...
      assignedDoctor: patient.profile?.assignedDoctor ? {
        ...
        specialization: patient.profile.assignedDoctor.profile.specialization
      } : null,
    ...
    const doctorsData = doctors.map(doctor => {
      const workload = doctorWorkload.find(w => w.doctorId.toString() === doctor._id.toString())
      return {
        id: doctor._id,
        name: doctor.profile.name,
        specialization: doctor.profile.specialization,
